...
function upload(file, settings) {
	...
	function Uploader(file, settings) {
		...
		settings.ajax_base = settings.ajax_base || "/upload";
...
//					xhr.setRequestHeader("x-amz-acl", "public-read");
			...
			var handler = function(e, is_head) {
				...
				if(e.target.status / 100 == 2) {
					...
					u.list_parts(function(parts) {
						...
						var next_chunk = u.get_next_chunk();
						console.log("NEXT CHUNK >> ", next_chunk);
						u.set_state("processing");
						u.upload_chunk(next_chunk);
					});
				} else {
		...
		this.get_list_signature(function(signature, date) {
			...
			var handler = function(e) {
				...
				$(xml).find('Part').each(function() {
					...
					} else if(part_number == num_chunks && size != u.file.size % u.settings.chunk_size) {
	...
	Uploader.prototype.get_chunk_signature = function(chunk, callback) {
		var u = this;
		if(u._chunk_signatures && u._chunk_signatures[chunk]) {
			callback(u._chunk_signatures[chunk][0], u._chunk_signatures[chunk][1]);
			return;
		}
		var xhr = new XMLHttpRequest();
		var handler = function(e) {
